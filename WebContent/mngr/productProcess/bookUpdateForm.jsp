	...
	<div id="bookUpdateForm" class="box">
		<ul>
			<li><label for="book_kind">분류선택</label> <select id="book_kind"
				name="book_kind">
					<option value="100">
						<c:if test="${book_kind==100 }">selected</c:if> 문학
					</option>
					<option value="200">
						<c:if test="${book_kind == 200 }">selected</c:if> 외국어
					</option>
					<option value="300">
						<c:if test="${book_kind == 300 }">selected</c:if> 컴퓨터
					</option>
			</select>
			...
			<li><label for="publishing_date">출판일</label>
				<div id="publishing_date">
					...
					<fmt:formatDate value="nowTimeStr" pattern="yyyy-MM-dd"
						value="${nowTime }" />
			...
			<li><label for="discont_rate">할인율</label> <input
